package com.aliyun.openservices.chlde.log.producer.internals.hash;
...
public class FingerPrint {
    ...
    private static long hashLength0to16(byte[] bytes, int offset, int length) {
        ...
        if (length >= 4) {
            ...
            return hashLength16(length + (a << 3), LittleEndianByteArray.load32(bytes, offset + length - 4) & 0xFFFFFFFFL, mul);
        }
        if (length > 0) {
            byte a = bytes[offset];
